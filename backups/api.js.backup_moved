  ...
  try {
    ...
    if (path.startsWith('/doctors')) {
      ...
      if (path === '/doctors/on-leave' && method === 'GET') {
        ...
        const result = await sql(['SELECT t2.name AS "NamaDokter", t2.specialty AS "Spesialis", t1.start_date AS "TanggalMulaiCuti", t1.end_date AS "TanggalSelesaiCuti" FROM leave_data t1 JOIN doctors t2 ON t1.doctor_id = t2.id WHERE t1.end_date >= ', ' ORDER BY t1.start_date ASC'], today);
      ...
      if ((path === '/doctors' || path === '/doctors/') && method === 'GET') {
        ...
        const searchFilter = search
          ? sql(['WHERE name ILIKE ', ' OR specialty ILIKE ', ' '], '%' + search + '%', '%' + search + '%')
          : sql(['']);

        const [countResult] = await sql(['SELECT COUNT(*) FROM doctors ', ' '], searchFilter);
        const doctors = await sql(['SELECT * FROM doctors ', ' ORDER BY name LIMIT ', ' OFFSET ', ' '], searchFilter, limit, offset);
      ...
      if ((path === '/doctors' || path === '/doctors/') && method === 'POST') {
        ...
        const [newDoctor] = await sql(['INSERT INTO doctors(name, specialty, image_url, schedule, updated_at) VALUES(', ', ', ', ', ', ', NOW()) RETURNING *'], name, specialty, image_url || '', schedule || '{}');
