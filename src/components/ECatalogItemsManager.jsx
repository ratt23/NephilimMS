...
import { Plus, Edit, Trash2, Save, X, Upload, Image as ImageIcon, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function ECatalogItemsManager() {
    ...
    async function toggleCategoryVisibility(categoryId) {
        try {
            ...
            const categoryName = CATEGORIES.find(c => c.id === categoryId)?.label;
            alert(`${categoryName} ${newVisibility[categoryId] ? 'ditampilkan' : 'disembunyikan (Coming Soon)'}`);
        } catch (err) {
            console.error('Error toggling category visibility:', err);
            alert('Gagal mengupdate visibilitas kategori');
            // Revert on error
            loadSettings();
        }
    }
