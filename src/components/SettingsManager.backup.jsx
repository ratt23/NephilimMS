    ...
    const loadSettings = async () => {
        ...
        try {
            ...
            setConfig({
                ...
                category_covers: (() => {
                    const loaded = safeParse('category_covers', getVal('category_covers'), {});
                    const merged = { ...loaded };
                    // Smart Merge: Ensure every default category has a cover entry
                    DEFAULT_CATEGORIES.forEach(cat => {
                        if (!merged[cat.id]) {
                            merged[cat.id] = '/asset/categories/placeholder.svg';
                        }
                    });
                    return merged;
                })(),
                category_visibility: {
                    ...
                    ...safeParse('category_visibility', getVal('category_visibility'), {})
                },
                ecatalog_categories: (() => {
                    const loaded = safeParse('ecatalog_categories', getVal('ecatalog_categories'), DEFAULT_CATEGORIES);
                    const loadedIds = new Set(loaded.map(c => c.id));
                    // Smart Merge: Add defaults that are missing from loaded config (e.g. new 'radiology')
                        ...
                        <form onSubmit={handleSave} className="space-y-4"> {/* Space y reduced, handle spacing in Accordion */}
                            ...
                            <AccordionSection title="eCatalog Category Covers">
                                <div className="space-y-4">
                                    ...
                                    {(config.ecatalog_categories || []).map(category => (
                            ...
                            <AccordionSection title="Category Visibility">
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {(config.ecatalog_categories || []).map(cat => (
